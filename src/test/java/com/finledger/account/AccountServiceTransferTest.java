...
import com.finledger.transaction.TransactionRepository;
...
public class AccountServiceTransferTest extends BaseIntegrationTest {
    ...
    void create_usecase_two_transaction_when_transfer() {
        ...
        List<Transaction> fromTransaction = transactionRepository.findByAccountIdOrderByCreatedDtDesc(fromId);
        List<Transaction> toTransaction   = transactionRepository.findByAccountIdOrderByCreatedDtDesc(toId);

        assertThat(fromTransaction).hasSize(1);
        assertThat(toTransaction).hasSize(1);
    }
}
